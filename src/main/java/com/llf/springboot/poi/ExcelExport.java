package com.llf.springboot.poi;
...
//    private HSSFWorkbook wb = null;
    ...
    public void writeExcel(String titleColumn[], String titleName[], int titleSize[], List<?> dataList) {
        ...
        try {
            ...
            // 设置标题
            Row titleRow = wb.getSheet(sheetName).createRow(0);
            ...
            ts = (HSSFCellStyle) setFontAndBorder(ts, titleFontType, (short) 16);
            ts = (HSSFCellStyle) setColor(ts, titleBackColor, (short) 10);
            //垂直居中
            ts.setAlignment(HorizontalAlignment.CENTER); // 创建一个居中格式
            //设置一个边框
            ts.setBorderTop(BorderStyle.DASH_DOT);
            Cell cell1 = titleRow.createCell(0);
            cell1.setCellStyle(ts);
            cell1.setCellValue("测试");

            //写入excel的表头  
            Row titleNameRow = wb.getSheet(sheetName).createRow(1);
            //设置样式  
            ...
            if (dataList != null && dataList.size() > 0) {
                ...
                if (titleColumn.length > 0) {
                    for (int rowIndex = 1; rowIndex <= dataList.size(); rowIndex++) {
                        ...
                        Row dataRow = wb.getSheet(sheetName).createRow(rowIndex + 1);
            ...
            e.printStackTrace();
            throw new ExcelExportException("创建失败");
        } finally {
            if (out != null) {
                try {
                    out.close();
     ...
     * @param dataList 数据
     */
    public void writeExcelC(List<?> dataList, Class aClass) {
        ...
        try {
            ...
            // 合并单元格：参数：起始行, 终止行, 起始列, 终止列
            CellRangeAddress cra = new CellRangeAddress(0, 0, 0, objLength - 1);
            sheet.addMergedRegion(cra);
            //注意：边框样式需要重新设置一下
            RegionUtil.setBorderTop(BorderStyle.THIN, cra, sheet);
            ...
            ts = setFontAndBorder(ts, titleFontType, (short) 16);
            ts = setColor(ts, titleBackColor, (short) 10);
            //垂直居中
            ts.setAlignment(HorizontalAlignment.CENTER); // 创建一个居中格式
            //设置一个边框
            ts.setBorderTop(BorderStyle.DASH_DOT);
            ...
            titleStyle = setFontAndBorder(titleStyle, titleFontType, (short) titleFontSize);
            ...
            for (int i = 0; i < declaredFields.length; i++) {
                ...
                if (annotation != null) {
                    ...
                    sheet.setColumnWidth(i - 1, width * 256);    //设置宽度
                    Cell cell = titleNameRow.createCell(i - 1);
                    cell.setCellStyle(titleStyle);
...
//                titleStyle = setFontAndBorder(titleStyle, contentFontType, (short) contentFontSize);
                ...
                if (objLength > 0) {
                    ...
                    for (int rowIndex = 1; rowIndex <= dataList.size(); rowIndex++) {
                        ...
                        for (int columnIndex = 0; columnIndex < declaredFields.length; columnIndex++) {
                            ...
                            if (annotation != null) {
                                ...
                                String data = field.get(obj) == null ? "" : object.toString();
                                Cell cell = dataRow.createCell(columnIndex - 1);
                                if (data != null && !"".equals(data)) {
                                    if ("int".equals(returnType)) {
                                        cell.setCellValue(Integer.parseInt(data));
                                    } else if ("long".equals(returnType)) {
...
//            stylea.setcolor((short) index, (byte) r, (byte) g, (byte) b);
